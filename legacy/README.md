...
> This is the maintained Python application. Paths in this document are relative
> to `legacy/` unless they describe the public installer or Git repository.
> From the repository root, use `./legacy/threadhop` (or the compatibility
> `./threadhop` symlink) and run tests against `legacy/tests/`.
